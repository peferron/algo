...
const tests: {frequencies: number[], codes: string[]}[] = [
    {
        frequencies: [30, 70],
        codes: ['0', '1']
    },
...
for (const test of tests) {
    const actual = encode(test.frequencies);
    assert.deepStrictEqual(actual, test.codes, `For frequencies ${test.frequencies}, ` +
        `expected codes to be ${test.codes}, but were ${actual}`);
}
